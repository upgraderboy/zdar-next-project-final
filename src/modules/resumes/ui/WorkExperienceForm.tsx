...
export default function WorkExperienceForm({
  ...
  console.log(resumeData.workExperiences);
  const form = useForm<WorkExperienceValues>({
    resolver: zodResolver(workExperienceSchema),
    defaultValues: {
      workExperiences: (resumeData.workExperiences || []).map((exp) => ({
        ...exp,
        startDate: exp.startDate ? new Date(exp.startDate).toISOString().split("T")[0] : undefined,
  ...
  useEffect(() => {
    setResumeData((prev) => ({
      ...prev,
      workExperiences: watchedWorkExperiences?.filter((exp) => exp !== undefined) || [],
    }));
  }, [watchedWorkExperiences, setResumeData]);
